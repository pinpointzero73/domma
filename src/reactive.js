 ...
 * can invalidate exactly the computations that care, and nothing else.
 *
 * This replaces the "re-evaluate every computed and deep-compare the results"
 * strategy in component-factory.js with a graph walk over real dependencies.
 *
 ...
 * (`model.set({ a, b, c })`, a rapid sequence of keystrokes) collapses into one
 * propagation pass and, downstream, one render.
 *
 * Usage:
 *
...
export class DepMap {
    ...
    /** Drop every Dep — used when the owning model is destroyed. */
    clear() {
...
export class Computation {
     ...
     * the previous one. Uses `utils.isEqual`, so structurally-identical objects
    ...
    recompute() {
        ...
        return { changed: !utils.isEqual(previous, this.value), value: this.value };
    }
 ...
 *
 * Reads are tracked one level deep, which matches the granularity at which
 * Domma Models notify (per top-level field), so nested proxying is unnecessary.
 *
 * @param {Object}   target            Object to wrap (typically a model's data).
 ...
 *                                     target directly — letting a Model keep its
 *                                     validation and change-notification path.
 * @returns {Proxy}
 */

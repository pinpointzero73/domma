...
help:
	...
	@echo ""
	@echo "Release  - the usual route is the Actions tab:"
	@echo "  Actions > Cut a release > patch/minor/major   (notes in docs/NEXT_RELEASE.md first)"
	@echo "  It builds, checks, commits, tags, publishes, cuts the GitHub release, then"
	@echo "  opens the re-pin PR in domma-cms."
	@echo ""
	@echo "Release by hand  (v$(VERSION) - run in this order):"
	...
	@echo "                        the tag publishes to npm and opens the domma-cms PR"
	@echo "  (make release-npm publishes from here instead - only when CI cannot, and"
	@echo "   then the tag's publish run fails and no domma-cms PR is opened)"
	@echo ""

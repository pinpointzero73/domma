...
The **Corporate** family is a consummate professional executive theme suite designed for high-trust enterprise, financial, and SaaS applications:

- `corporate-light`: Clean architectural alabaster background (`#f8fafc`), pure white card surfaces (`#ffffff`), deep ink-navy typography (`#0f172a`), and executive deep navy primary (`#1e40af`).
- `corporate-dark`: Deep obsidian midnight canvas (`#0b0f19`), elevated navy-slate card surfaces (`#151c2c`), ice-white typography (`#f8fafc`), and refined sapphire primary (`#2563eb`).

```javascript
Domma.theme.set('corporate-light');
Domma.theme.set('corporate-dark');
```

### Admin family (standalone)

The **Admin** family is a dedicated dashboard theme set. It has no light/dark variant - it is its own family of nine themes, a 3 × 3 matrix of **finish** × **accent**:

- **Finishes:** `smooth` (uniform dark surfaces) · `slate` (high-visibility mid-level matte slate) · `sharp` (dark chrome + tinted-light workspace)
...
Domma.theme.set('admin-slate-steel');
Domma.theme.getBase(); // 'admin-slate'
```

The nine CSS files are generated by `scripts/generate-admin-themes.js` (run via
